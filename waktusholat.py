...
def request(namakota):
    ...
    if best_match[1] >= limitation:
        kota_id = next((kota["id"] for kota in idkota["kota"] if kota["nama"] == best_match[0]), None) # Cari ID kota yang cocok dengan nama kota yang mendekati
        if kota_id:
            print("ID dari", best_match[0], "adalah:", kota_id)
            jadwalkota = requests.get(f"https://api.banghasan.com/sholat/format/json/jadwal/kota/{kota_id}/tanggal/{now.year}-{now.month}-{now.day}").json()
...
request("pematang siantar") #Input Kota Disini
